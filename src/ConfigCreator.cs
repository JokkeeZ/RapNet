...
internal sealed class ConfigCreator : IDisposable {
    ...
    private string Padding => 
        !Program.GetAppSettings().UseSpaces ? new string('\t', _paddingSize) : string.Join(" ", Enumerable.Repeat(" ", _paddingSize));

    internal ConfigCreator(Config config, string path) 
    {
        _config = config;
        _path = path;
    ...
    private void WriteEntriesFromClass(RapClass child, StreamWriter? headerWriter = null) 
    {
        ...
        child.DeleteStatements.ForEach(o => writer.WriteLine(Padding + o.ToConfigFormat()));
        child.ExternalClasses.ForEach(o => writer.WriteLine(Padding + o.ToConfigFormat()));
